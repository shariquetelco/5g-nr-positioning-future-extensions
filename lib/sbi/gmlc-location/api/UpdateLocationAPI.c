...
void UpdateLocationAPI_updateLocation(apiClient_t	*apiClient,
	...
	// create the path
	long sizeOfPath = strlen("/location-update") + 1;
	char *localVarPath = malloc(sizeOfPath);
	snprintf(localVarPath, sizeOfPath, "/location-update");




	// Body Param
	cJSON *localVarSingleItemJSON_loc_update_data = NULL;
	if(loc_update_data != NULL) {
		// string
		localVarSingleItemJSON_loc_update_data =
			loc_update_data_convertToJSON(loc_update_data);
	...
	free(localVarPath);
	if(localVarSingleItemJSON_loc_update_data) {
		cJSON_Delete(localVarSingleItemJSON_loc_update_data);
		localVarSingleItemJSON_loc_update_data = NULL;
	}
