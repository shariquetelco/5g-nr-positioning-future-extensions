...
char *vertical_direction_ToString(vertical_direction_e vertical_direction) {
	const char *vertical_directionArray[] =
	{ "NULL", "UPWARD", "DOWNWARD" };
	size_t sizeofArray = sizeof(vertical_directionArray) /
	                     sizeof(vertical_directionArray[0]);
...
vertical_direction_e vertical_direction_FromString(char *vertical_direction) {
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(vertical_direction,
		          vertical_directionArray[stringToReturn]) == 0)
		{
