...
location_data_t *location_data_create(char *gpsi, char *supi,
	...
	location_data_local_var->accepted_periodic_event_info =
	...
	location_data_local_var->related_applicationlayer_id =
...
void location_data_free(location_data_t *location_data) {
	...
	if(location_data->gpsi) {
		free(location_data->gpsi);
		location_data->gpsi = NULL;
	}
	if(location_data->supi) {
		free(location_data->supi);
		location_data->supi = NULL;
	}
	if(location_data->location_estimate) {
		geographic_area_free(location_data->location_estimate);
		location_data->location_estimate = NULL;
	}
	if(location_data->civic_address) {
		civic_address_free(location_data->civic_address);
		location_data->civic_address = NULL;
	}
	if(location_data->local_location_estimate) {
		local_area_free(location_data->local_location_estimate);
		location_data->local_location_estimate = NULL;
	}
	if(location_data->timestamp_of_location_estimate) {
		free(location_data->timestamp_of_location_estimate);
		location_data->timestamp_of_location_estimate = NULL;
	}
	if(location_data->positioning_data_list) {
		list_for_each(location_data->positioning_data_list, node) {
			positioning_method_and_usage_free(node->data);
		}
		list_free(location_data->positioning_data_list);
		location_data->positioning_data_list = NULL;
	}
	if(location_data->gnss_positioning_data_list) {
		list_for_each(location_data->gnss_positioning_data_list, node) {
			gnss_positioning_method_and_usage_free(node->data);
		}
		list_free(location_data->gnss_positioning_data_list);
		location_data->gnss_positioning_data_list = NULL;
	}
	if(location_data->ue_velocity) {
		velocity_estimate_free(location_data->ue_velocity);
		location_data->ue_velocity = NULL;
	}
	if(location_data->ldr_reference) {
		free(location_data->ldr_reference);
		location_data->ldr_reference = NULL;
	}
	if(location_data->serving_lmf_identification) {
		free(location_data->serving_lmf_identification);
		location_data->serving_lmf_identification = NULL;
	}
	if(location_data->location_privacy_ver_result) {
		location_privacy_ver_result_free(
			location_data->location_privacy_ver_result);
		location_data->location_privacy_ver_result = NULL;
	}
	if(location_data->achieved_qos) {
		minor_location_qo_s_free(location_data->achieved_qos);
		location_data->achieved_qos = NULL;
	}
	if(location_data->accepted_periodic_event_info) {
		periodic_event_info_free(
			location_data->accepted_periodic_event_info);
		location_data->accepted_periodic_event_info = NULL;
	}
	if(location_data->ha_gnss_metrics) {
		high_accuracy_gnss_metrics_free(location_data->ha_gnss_metrics);
		location_data->ha_gnss_metrics = NULL;
	}
	if(location_data->related_applicationlayer_id) {
		free(location_data->related_applicationlayer_id);
		location_data->related_applicationlayer_id = NULL;
	}
	if(location_data->distance_direction) {
		range_direction_free(location_data->distance_direction);
		location_data->distance_direction = NULL;
	}
	if(location_data->_2d_relative_location) {
		model_2_d_relative_location_free(
			location_data->_2d_relative_location);
		location_data->_2d_relative_location = NULL;
	}
	if(location_data->_3d_relative_location) {
		model_3_d_relative_location_free(
			location_data->_3d_relative_location);
		location_data->_3d_relative_location = NULL;
	}
	if(location_data->relative_velocity) {
		velocity_estimate_free(location_data->relative_velocity);
		location_data->relative_velocity = NULL;
	}
	free(location_data);
}
...
cJSON *location_data_convertToJSON(location_data_t *location_data) {
	...
	if(location_data == NULL) {
		printf("location_data_convertToJSON() failed [LocationData]");
		return NULL;
	}
	...
	if(location_data->gpsi) {
		if(cJSON_AddStringToObject(item, "gpsi",
		                           location_data->gpsi) == NULL)
		{
			printf("location_data_convertToJSON() failed [gpsi]");
			goto end;
		}
	}
	...
	if(location_data->supi) {
		if(cJSON_AddStringToObject(item, "supi",
		                           location_data->supi) == NULL)
		{
			printf("location_data_convertToJSON() failed [supi]");
			goto end;
		}
	}
	...
	if(location_data->location_estimate) {
		cJSON *location_estimate_local_JSON =
			geographic_area_convertToJSON(
				location_data->location_estimate);
		if(location_estimate_local_JSON == NULL) {
			printf(
				"location_data_convertToJSON() failed [location_estimate]");
			goto end;
		}
	...
	if(location_data->civic_address) {
		cJSON *civic_address_local_JSON = civic_address_convertToJSON(
			location_data->civic_address);
		if(civic_address_local_JSON == NULL) {
			printf(
				"location_data_convertToJSON() failed [civic_address]");
			goto end;
		}
	...
	if(location_data->local_location_estimate) {
		cJSON *local_location_estimate_local_JSON =
			local_area_convertToJSON(
				location_data->local_location_estimate);
		if(local_location_estimate_local_JSON == NULL) {
			printf(
				"location_data_convertToJSON() failed [local_location_estimate]");
			goto end;
		}
	...
	if(location_data->is_age_of_location_estimate) {
		...
		{
			printf(
				"location_data_convertToJSON() failed [age_of_location_estimate]");
			goto end;
		}
	}
	...
	if(location_data->timestamp_of_location_estimate) {
		...
		{
			printf(
				"location_data_convertToJSON() failed [timestamp_of_location_estimate]");
			goto end;
		}
	}
	...
	if(location_data->positioning_data_list) {
		...
		if(positioning_data_listList == NULL) {
			printf(
				"location_data_convertToJSON() failed [positioning_data_list]");
			goto end;
		}
		list_for_each(location_data->positioning_data_list, node) {
			cJSON *itemLocal =
	...
	if(location_data->gnss_positioning_data_list) {
		...
		if(gnss_positioning_data_listList == NULL) {
			printf(
				"location_data_convertToJSON() failed [gnss_positioning_data_list]");
			goto end;
		}
		list_for_each(location_data->gnss_positioning_data_list, node) {
			cJSON *itemLocal =
	   ...
	   accuracy_fulfilment_indicator_NULL)
	{
		...
		{
			printf(
				"location_data_convertToJSON() failed [accuracy_fulfilment_indicator]");
			goto end;
		}
	}
	...
	if(location_data->ue_velocity) {
		cJSON *ue_velocity_local_JSON = velocity_estimate_convertToJSON(
			location_data->ue_velocity);
		if(ue_velocity_local_JSON == NULL) {
			printf(
				"location_data_convertToJSON() failed [ue_velocity]");
			goto end;
		}
	...
	if(location_data->ldr_reference) {
		if(cJSON_AddStringToObject(item, "ldrReference",
		                           location_data->ldr_reference) ==
		   NULL)
		{
			printf(
				"location_data_convertToJSON() failed [ldr_reference]");
			goto end;
		}
	}
	...
	if(location_data->is_altitude) {
		if(cJSON_AddNumberToObject(item, "altitude",
		                           location_data->altitude) == NULL)
		{
			printf("location_data_convertToJSON() failed [altitude]");
			goto end;
		}
	}
	...
	if(location_data->serving_lmf_identification) {
		...
		{
			printf(
				"location_data_convertToJSON() failed [serving_lmf_identification]");
			goto end;
		}
	}
	...
	if(location_data->location_privacy_ver_result) {
		...
		if(location_privacy_ver_result_local_JSON == NULL) {
			printf(
				"location_data_convertToJSON() failed [location_privacy_ver_result]");
			goto end;
		}
	...
	if(location_data->success_type != success_type_NULL) {
		if(cJSON_AddStringToObject(item, "successType",
		                           success_type_ToString(location_data->
		                                                 success_type))
		   == NULL)
		{
			printf(
				"location_data_convertToJSON() failed [success_type]");
			goto end;
		}
	}
	...
	if(location_data->achieved_qos) {
		cJSON *achieved_qos_local_JSON =
			minor_location_qo_s_convertToJSON(
				location_data->achieved_qos);
		if(achieved_qos_local_JSON == NULL) {
			printf(
				"location_data_convertToJSON() failed [achieved_qos]");
			goto end;
		}
	...
	if(location_data->is_direct_report_ind) {
		if(cJSON_AddBoolToObject(item, "directReportInd",
		                         location_data->direct_report_ind) ==
		   NULL)
		{
			printf(
				"location_data_convertToJSON() failed [direct_report_ind]");
			goto end;
		}
	}
	...
	if(location_data->accepted_periodic_event_info) {
		...
		if(accepted_periodic_event_info_local_JSON == NULL) {
			printf(
				"location_data_convertToJSON() failed [accepted_periodic_event_info]");
			goto end;
		}
	...
	if(location_data->ha_gnss_metrics) {
		cJSON *ha_gnss_metrics_local_JSON =
			high_accuracy_gnss_metrics_convertToJSON(
				location_data->ha_gnss_metrics);
		if(ha_gnss_metrics_local_JSON == NULL) {
			printf(
				"location_data_convertToJSON() failed [ha_gnss_metrics]");
			goto end;
		}
	...
	if(location_data->los_nlos_measure_ind != los_nlos_measure_ind_NULL) {
		...
		{
			printf(
				"location_data_convertToJSON() failed [los_nlos_measure_ind]");
			goto end;
		}
	}

	if(location_data->indoor_outdoor_ind != indoor_outdoor_ind_NULL) {
		...
		{
			printf(
				"location_data_convertToJSON() failed [indoor_outdoor_ind]");
			goto end;
		}
	}
	...
	if(location_data->related_applicationlayer_id) {
		...
		{
			printf(
				"location_data_convertToJSON() failed [related_applicationlayer_id]");
			goto end;
		}
	}
	...
	if(location_data->distance_direction) {
		cJSON *distance_direction_local_JSON =
			range_direction_convertToJSON(
				location_data->distance_direction);
		if(distance_direction_local_JSON == NULL) {
			printf(
				"location_data_convertToJSON() failed [distance_direction]");
			goto end;
		}
	...
	if(location_data->_2d_relative_location) {
		...
		if(_2d_relative_location_local_JSON == NULL) {
			printf(
				"location_data_convertToJSON() failed [_2d_relative_location]");
			goto end;
		}
	...
	if(location_data->_3d_relative_location) {
		...
		if(_3d_relative_location_local_JSON == NULL) {
			printf(
				"location_data_convertToJSON() failed [_3d_relative_location]");
			goto end;
		}
	...
	if(location_data->relative_velocity) {
		cJSON *relative_velocity_local_JSON =
			velocity_estimate_convertToJSON(
				location_data->relative_velocity);
		if(relative_velocity_local_JSON == NULL) {
			printf(
				"location_data_convertToJSON() failed [relative_velocity]");
			goto end;
		}
...
location_data_t *location_data_parseFromJSON(cJSON *location_dataJSON) {
	location_data_t *location_data_local_var = NULL;
	lnode_t *node = NULL;
	cJSON *gpsi = NULL;
	cJSON *supi = NULL;
	...
	gpsi = cJSON_GetObjectItemCaseSensitive(location_dataJSON, "gpsi");
	if(gpsi) {
		if(!cJSON_IsString(gpsi) &&
		   !cJSON_IsNull(gpsi))
		{
			printf("location_data_parseFromJSON() failed [gpsi]");
			goto end;
		}
	}

	supi = cJSON_GetObjectItemCaseSensitive(location_dataJSON, "supi");
	if(supi) {
		if(!cJSON_IsString(supi) &&
		   !cJSON_IsNull(supi))
		{
			printf("location_data_parseFromJSON() failed [supi]");
			goto end;
		}
	}

	location_estimate = cJSON_GetObjectItemCaseSensitive(location_dataJSON,
	                                                     "locationEstimate");
	...
	civic_address = cJSON_GetObjectItemCaseSensitive(location_dataJSON,
	                                                 "civicAddress");
	if(civic_address) {
	...
	local_location_estimate = cJSON_GetObjectItemCaseSensitive(
		location_dataJSON, "localLocationEstimate");
	...
	age_of_location_estimate = cJSON_GetObjectItemCaseSensitive(
		location_dataJSON, "ageOfLocationEstimate");
	if(age_of_location_estimate) {
		if(!cJSON_IsNumber(age_of_location_estimate)) {
			printf(
				"location_data_parseFromJSON() failed [age_of_location_estimate]");
			goto end;
		}
	}

	timestamp_of_location_estimate = cJSON_GetObjectItemCaseSensitive(
		location_dataJSON, "timestampOfLocationEstimate");
	if(timestamp_of_location_estimate) {
		...
		{
			printf(
				"location_data_parseFromJSON() failed [timestamp_of_location_estimate]");
			goto end;
		}
	}

	positioning_data_list = cJSON_GetObjectItemCaseSensitive(
		location_dataJSON, "positioningDataList");
	if(positioning_data_list) {
		cJSON *positioning_data_list_local = NULL;
		if(!cJSON_IsArray(positioning_data_list)) {
			printf(
				"location_data_parseFromJSON() failed [positioning_data_list]");
			goto end;
		}
	...
	accuracy_fulfilment_indicator = cJSON_GetObjectItemCaseSensitive(
		location_dataJSON, "accuracyFulfilmentIndicator");
	if(accuracy_fulfilment_indicator) {
		if(!cJSON_IsString(accuracy_fulfilment_indicator)) {
			printf(
				"location_data_parseFromJSON() failed [accuracy_fulfilment_indicator]");
			goto end;
		}
	...
	ue_velocity = cJSON_GetObjectItemCaseSensitive(location_dataJSON,
	                                               "ueVelocity");
	if(ue_velocity) {
	...
	ldr_reference = cJSON_GetObjectItemCaseSensitive(location_dataJSON,
	                                                 "ldrReference");
	if(ldr_reference) {
		...
		{
			printf(
				"location_data_parseFromJSON() failed [ldr_reference]");
			goto end;
		}
	}

	altitude = cJSON_GetObjectItemCaseSensitive(location_dataJSON,
	                                            "altitude");
	if(altitude) {
		if(!cJSON_IsNumber(altitude)) {
			printf("location_data_parseFromJSON() failed [altitude]");
			goto end;
		}
	}

	serving_lmf_identification = cJSON_GetObjectItemCaseSensitive(
		location_dataJSON, "servingLMFIdentification");
	if(serving_lmf_identification) {
		if(!cJSON_IsString(serving_lmf_identification) &&
		   !cJSON_IsNull(serving_lmf_identification))
		{
			printf(
				"location_data_parseFromJSON() failed [serving_lmf_identification]");
			goto end;
		}
	}

	location_privacy_ver_result = cJSON_GetObjectItemCaseSensitive(
		location_dataJSON, "locationPrivacyVerResult");
	...
	success_type = cJSON_GetObjectItemCaseSensitive(location_dataJSON,
	                                                "successType");
	if(success_type) {
		if(!cJSON_IsString(success_type)) {
			printf(
				"location_data_parseFromJSON() failed [success_type]");
			goto end;
		}
	...
	achieved_qos = cJSON_GetObjectItemCaseSensitive(location_dataJSON,
	                                                "achievedQos");
	if(achieved_qos) {
	...
	direct_report_ind = cJSON_GetObjectItemCaseSensitive(location_dataJSON,
	                                                     "directReportInd");
	if(direct_report_ind) {
		if(!cJSON_IsBool(direct_report_ind)) {
			printf(
				"location_data_parseFromJSON() failed [direct_report_ind]");
			goto end;
		}
	}

	accepted_periodic_event_info = cJSON_GetObjectItemCaseSensitive(
		location_dataJSON, "acceptedPeriodicEventInfo");
	...
	ha_gnss_metrics = cJSON_GetObjectItemCaseSensitive(location_dataJSON,
	                                                   "haGnssMetrics");
	if(ha_gnss_metrics) {
	...
	los_nlos_measure_ind = cJSON_GetObjectItemCaseSensitive(
		location_dataJSON, "losNlosMeasureInd");
	if(los_nlos_measure_ind) {
		if(!cJSON_IsString(los_nlos_measure_ind)) {
			printf(
				"location_data_parseFromJSON() failed [los_nlos_measure_ind]");
			goto end;
		}
	...
	indoor_outdoor_ind = cJSON_GetObjectItemCaseSensitive(location_dataJSON,
	                                                      "indoorOutdoorInd");
	if(indoor_outdoor_ind) {
		if(!cJSON_IsString(indoor_outdoor_ind)) {
			printf(
				"location_data_parseFromJSON() failed [indoor_outdoor_ind]");
			goto end;
		}
	...
	related_applicationlayer_id = cJSON_GetObjectItemCaseSensitive(
		location_dataJSON, "relatedApplicationlayerId");
	if(related_applicationlayer_id) {
		if(!cJSON_IsString(related_applicationlayer_id) &&
		   !cJSON_IsNull(related_applicationlayer_id))
		{
			printf(
				"location_data_parseFromJSON() failed [related_applicationlayer_id]");
			goto end;
		}
	}

	distance_direction = cJSON_GetObjectItemCaseSensitive(location_dataJSON,
	                                                      "distanceDirection");
	...
	_2d_relative_location = cJSON_GetObjectItemCaseSensitive(
		location_dataJSON, "2dRelativeLocation");
	...
	_3d_relative_location = cJSON_GetObjectItemCaseSensitive(
		location_dataJSON, "3dRelativeLocation");
	...
	relative_velocity = cJSON_GetObjectItemCaseSensitive(location_dataJSON,
	                                                     "relativeVelocity");
	...
	location_data_local_var = location_data_create(
		gpsi &&
...
location_data_t *location_data_copy(location_data_t	*dst,
                                    location_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;

	item = location_data_convertToJSON(src);
	if(!item) {
		printf("location_data_convertToJSON() failed");
		return NULL;
	}
	...
	location_data_free(dst);
	dst = location_data_parseFromJSON(item);
	cJSON_Delete(item);
