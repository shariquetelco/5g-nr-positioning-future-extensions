...
event_notify_data_t *event_notify_data_create(char *gpsi, char *supi,
                                              char *ldr_reference,
...
{
	event_notify_data_t *event_notify_data_local_var =
		malloc(sizeof(event_notify_data_t));
	...
	event_notify_data_local_var->event_notify_data_type =
	...
	event_notify_data_local_var->lmf_identification = lmf_identification;
	event_notify_data_local_var->amf_id = amf_id;
	...
	event_notify_data_local_var->target_node = target_node;
	event_notify_data_local_var->accuracy_fulfilment_indicator =
		accuracy_fulfilment_indicator;
	event_notify_data_local_var->failure_cause = failure_cause;
...
void event_notify_data_free(event_notify_data_t *event_notify_data) {
	...
	if(event_notify_data->positioning_data_list) {
		list_for_each(event_notify_data->positioning_data_list, node) {
			positioning_method_and_usage_free(node->data);
		}
		list_free(event_notify_data->positioning_data_list);
		event_notify_data->positioning_data_list = NULL;
	}
	if(event_notify_data->gnss_positioning_data_list) {
		list_for_each(event_notify_data->gnss_positioning_data_list,
		              node) {
			gnss_positioning_method_and_usage_free(node->data);
		}
		list_free(event_notify_data->gnss_positioning_data_list);
		event_notify_data->gnss_positioning_data_list = NULL;
	}
	if(event_notify_data->lmf_identification) {
		free(event_notify_data->lmf_identification);
		event_notify_data->lmf_identification = NULL;
	}
	if(event_notify_data->amf_id) {
		free(event_notify_data->amf_id);
		event_notify_data->amf_id = NULL;
	}
	...
	if(event_notify_data->target_node) {
		free(event_notify_data->target_node);
		event_notify_data->target_node = NULL;
	}
...
cJSON *event_notify_data_convertToJSON(event_notify_data_t *event_notify_data) {
	...
	if(event_notify_data == NULL) {
		printf(
			"event_notify_data_convertToJSON() failed [EventNotifyData]");
		return NULL;
	}
	...
	if(event_notify_data->gpsi) {
		if(cJSON_AddStringToObject(item, "gpsi",
		                           event_notify_data->gpsi) == NULL)
		{
			printf("event_notify_data_convertToJSON() failed [gpsi]");
			goto end;
		}
	}
	...
	if(event_notify_data->supi) {
		if(cJSON_AddStringToObject(item, "supi",
		                           event_notify_data->supi) == NULL)
		{
			printf("event_notify_data_convertToJSON() failed [supi]");
			goto end;
		}
	}
	...
	if(!event_notify_data->ldr_reference) {
		printf(
			"event_notify_data_convertToJSON() failed [ldr_reference]");
		return NULL;
	}
	...
	if(event_notify_data->event_notify_data_type ==
	   event_notify_data_type_NULL)
	{
		printf(
			"event_notify_data_convertToJSON() failed [event_notify_data_type]");
		return NULL;
	}
	...
	if(event_notify_data->location_estimate) {
		...
		if(location_estimate_local_JSON == NULL) {
			printf(
				"event_notify_data_convertToJSON() failed [location_estimate]");
			goto end;
		}
	...
	if(event_notify_data->civic_address) {
		...
		if(civic_address_local_JSON == NULL) {
			printf(
				"event_notify_data_convertToJSON() failed [civic_address]");
			goto end;
		}
	...
	if(event_notify_data->local_location_estimate) {
		...
		if(local_location_estimate_local_JSON == NULL) {
			printf(
				"event_notify_data_convertToJSON() failed [local_location_estimate]");
			goto end;
		}
	...
	if(event_notify_data->is_age_of_location_estimate) {
		...
		{
			printf(
				"event_notify_data_convertToJSON() failed [age_of_location_estimate]");
			goto end;
		}
	}
	...
	if(event_notify_data->timestamp_of_location_estimate) {
		...
		{
			printf(
				"event_notify_data_convertToJSON() failed [timestamp_of_location_estimate]");
			goto end;
		}
	}
	...
	if(event_notify_data->positioning_data_list) {
		...
		if(positioning_data_listList == NULL) {
			printf(
				"event_notify_data_convertToJSON() failed [positioning_data_list]");
			goto end;
		}
		list_for_each(event_notify_data->positioning_data_list, node) {
			cJSON *itemLocal =
	...
	if(event_notify_data->gnss_positioning_data_list) {
		...
		if(gnss_positioning_data_listList == NULL) {
			printf(
				"event_notify_data_convertToJSON() failed [gnss_positioning_data_list]");
			goto end;
		}
		list_for_each(event_notify_data->gnss_positioning_data_list,
		              node) {
	...
	if(event_notify_data->lmf_identification) {
		if(cJSON_AddStringToObject(item, "lmfIdentification",
		                           event_notify_data->lmf_identification)
		   == NULL)
		{
			printf(
				"event_notify_data_convertToJSON() failed [lmf_identification]");
			goto end;
		}
	}
	...
	if(event_notify_data->amf_id) {
		...
		{
			printf(
				"event_notify_data_convertToJSON() failed [amf_id]");
			goto end;
		}
	}

	if(event_notify_data->termination_cause != termination_cause_NULL) {
		...
		{
			printf(
				"event_notify_data_convertToJSON() failed [termination_cause]");
			goto end;
		}
	}
	...
	if(event_notify_data->velocity_estimate) {
		...
		if(velocity_estimate_local_JSON == NULL) {
			printf(
				"event_notify_data_convertToJSON() failed [velocity_estimate]");
			goto end;
		}
	...
	if(event_notify_data->is_altitude) {
		...
		{
			printf(
				"event_notify_data_convertToJSON() failed [altitude]");
			goto end;
		}
	}
	...
	if(event_notify_data->target_node) {
		if(cJSON_AddStringToObject(item, "targetNode",
		                           event_notify_data->target_node) ==
		   NULL)
		{
			printf(
				"event_notify_data_convertToJSON() failed [target_node]");
			goto end;
		}
	}

	if(event_notify_data->accuracy_fulfilment_indicator !=
	   accuracy_fulfilment_indicator_NULL)
	{
		...
		{
			printf(
				"event_notify_data_convertToJSON() failed [accuracy_fulfilment_indicator]");
			goto end;
		}
	}

	if(event_notify_data->failure_cause != failure_cause_NULL) {
		...
		{
			printf(
				"event_notify_data_convertToJSON() failed [failure_cause]");
			goto end;
		}
	}
	...
	if(event_notify_data->achieved_qos) {
		...
		if(achieved_qos_local_JSON == NULL) {
			printf(
				"event_notify_data_convertToJSON() failed [achieved_qos]");
			goto end;
		}
	...
	if(event_notify_data->ha_gnss_metrics) {
		...
		if(ha_gnss_metrics_local_JSON == NULL) {
			printf(
				"event_notify_data_convertToJSON() failed [ha_gnss_metrics]");
			goto end;
		}
	   ...
	   los_nlos_measure_ind_NULL)
	{
		...
		{
			printf(
				"event_notify_data_convertToJSON() failed [los_nlos_measure_ind]");
			goto end;
		}
	}
	...
	if(event_notify_data->is_up_loc_rep_stat_af) {
		...
		{
			printf(
				"event_notify_data_convertToJSON() failed [up_loc_rep_stat_af]");
			goto end;
		}
	}

	if(event_notify_data->indoor_outdoor_ind != indoor_outdoor_ind_NULL) {
		...
		{
			printf(
				"event_notify_data_convertToJSON() failed [indoor_outdoor_ind]");
			goto end;
		}
	}
	...
	if(event_notify_data->related_applicationlayer_id) {
		...
		{
			printf(
				"event_notify_data_convertToJSON() failed [related_applicationlayer_id]");
			goto end;
		}
	}
	...
	if(event_notify_data->distance_direction) {
		...
		if(distance_direction_local_JSON == NULL) {
			printf(
				"event_notify_data_convertToJSON() failed [distance_direction]");
			goto end;
		}
	...
	if(event_notify_data->_2d_relative_location) {
		...
		if(_2d_relative_location_local_JSON == NULL) {
			printf(
				"event_notify_data_convertToJSON() failed [_2d_relative_location]");
			goto end;
		}
	...
	if(event_notify_data->_3d_relative_location) {
		...
		if(_3d_relative_location_local_JSON == NULL) {
			printf(
				"event_notify_data_convertToJSON() failed [_3d_relative_location]");
			goto end;
		}
	...
	if(event_notify_data->relative_velocity) {
		...
		if(relative_velocity_local_JSON == NULL) {
			printf(
				"event_notify_data_convertToJSON() failed [relative_velocity]");
			goto end;
		}
...
event_notify_data_t *event_notify_data_parseFromJSON(
	...
	if(gpsi) {
		if(!cJSON_IsString(gpsi) &&
		   !cJSON_IsNull(gpsi))
		{
			printf("event_notify_data_parseFromJSON() failed [gpsi]");
			goto end;
		}
	}
	...
	if(supi) {
		if(!cJSON_IsString(supi) &&
		   !cJSON_IsNull(supi))
		{
			printf("event_notify_data_parseFromJSON() failed [supi]");
			goto end;
		}
	}
	...
	if(!ldr_reference) {
		printf(
			"event_notify_data_parseFromJSON() failed [ldr_reference]");
		goto end;
	}
	...
	event_notify_data_type = cJSON_GetObjectItemCaseSensitive(
		event_notify_dataJSON, "eventNotifyDataType");
	if(!event_notify_data_type) {
		printf(
			"event_notify_data_parseFromJSON() failed [event_notify_data_type]");
		goto end;
	}
	...
	location_estimate = cJSON_GetObjectItemCaseSensitive(
		event_notify_dataJSON, "locationEstimate");
	...
	local_location_estimate = cJSON_GetObjectItemCaseSensitive(
		event_notify_dataJSON, "localLocationEstimate");
	...
	age_of_location_estimate = cJSON_GetObjectItemCaseSensitive(
		event_notify_dataJSON, "ageOfLocationEstimate");
	if(age_of_location_estimate) {
		if(!cJSON_IsNumber(age_of_location_estimate)) {
			printf(
				"event_notify_data_parseFromJSON() failed [age_of_location_estimate]");
			goto end;
		}
	}

	timestamp_of_location_estimate = cJSON_GetObjectItemCaseSensitive(
		event_notify_dataJSON, "timestampOfLocationEstimate");
	if(timestamp_of_location_estimate) {
		...
		{
			printf(
				"event_notify_data_parseFromJSON() failed [timestamp_of_location_estimate]");
			goto end;
		}
	}

	positioning_data_list = cJSON_GetObjectItemCaseSensitive(
		event_notify_dataJSON, "positioningDataList");
	if(positioning_data_list) {
		cJSON *positioning_data_list_local = NULL;
		if(!cJSON_IsArray(positioning_data_list)) {
			printf(
				"event_notify_data_parseFromJSON() failed [positioning_data_list]");
			goto end;
		}
	...
	lmf_identification = cJSON_GetObjectItemCaseSensitive(
		event_notify_dataJSON, "lmfIdentification");
	if(lmf_identification) {
		if(!cJSON_IsString(lmf_identification) &&
		   !cJSON_IsNull(lmf_identification))
		{
			printf(
				"event_notify_data_parseFromJSON() failed [lmf_identification]");
			goto end;
		}
	}
	...
	if(amf_id) {
		...
		{
			printf(
				"event_notify_data_parseFromJSON() failed [amf_id]");
			goto end;
		}
	}

	termination_cause = cJSON_GetObjectItemCaseSensitive(
		event_notify_dataJSON, "terminationCause");
	if(termination_cause) {
		if(!cJSON_IsString(termination_cause)) {
			printf(
				"event_notify_data_parseFromJSON() failed [termination_cause]");
			goto end;
		}
	...
	velocity_estimate = cJSON_GetObjectItemCaseSensitive(
		event_notify_dataJSON, "velocityEstimate");
	...
	if(altitude) {
		if(!cJSON_IsNumber(altitude)) {
			printf(
				"event_notify_data_parseFromJSON() failed [altitude]");
			goto end;
		}
	}

	target_node = cJSON_GetObjectItemCaseSensitive(event_notify_dataJSON,
	                                               "targetNode");
	if(target_node) {
		...
		{
			printf(
				"event_notify_data_parseFromJSON() failed [target_node]");
			goto end;
		}
	}

	accuracy_fulfilment_indicator = cJSON_GetObjectItemCaseSensitive(
		event_notify_dataJSON, "accuracyFulfilmentIndicator");
	if(accuracy_fulfilment_indicator) {
		if(!cJSON_IsString(accuracy_fulfilment_indicator)) {
			printf(
				"event_notify_data_parseFromJSON() failed [accuracy_fulfilment_indicator]");
			goto end;
		}
	...
	failure_cause = cJSON_GetObjectItemCaseSensitive(event_notify_dataJSON,
	                                                 "failureCause");
	if(failure_cause) {
		if(!cJSON_IsString(failure_cause)) {
			printf(
				"event_notify_data_parseFromJSON() failed [failure_cause]");
			goto end;
		}
	...
	ha_gnss_metrics = cJSON_GetObjectItemCaseSensitive(
		event_notify_dataJSON, "haGnssMetrics");
	if(ha_gnss_metrics) {
	...
	los_nlos_measure_ind = cJSON_GetObjectItemCaseSensitive(
		event_notify_dataJSON, "losNlosMeasureInd");
	if(los_nlos_measure_ind) {
		if(!cJSON_IsString(los_nlos_measure_ind)) {
			printf(
				"event_notify_data_parseFromJSON() failed [los_nlos_measure_ind]");
			goto end;
		}
	...
	up_loc_rep_stat_af = cJSON_GetObjectItemCaseSensitive(
		event_notify_dataJSON, "upLocRepStatAf");
	if(up_loc_rep_stat_af) {
		if(!cJSON_IsNumber(up_loc_rep_stat_af)) {
			printf(
				"event_notify_data_parseFromJSON() failed [up_loc_rep_stat_af]");
			goto end;
		}
	}

	indoor_outdoor_ind = cJSON_GetObjectItemCaseSensitive(
		event_notify_dataJSON, "indoorOutdoorInd");
	if(indoor_outdoor_ind) {
		if(!cJSON_IsString(indoor_outdoor_ind)) {
			printf(
				"event_notify_data_parseFromJSON() failed [indoor_outdoor_ind]");
			goto end;
		}
	...
	_2d_relative_location = cJSON_GetObjectItemCaseSensitive(
		event_notify_dataJSON, "2dRelativeLocation");
	...
	_3d_relative_location = cJSON_GetObjectItemCaseSensitive(
		event_notify_dataJSON, "3dRelativeLocation");
	...
	relative_velocity = cJSON_GetObjectItemCaseSensitive(
		event_notify_dataJSON, "relativeVelocity");
	...
	event_notify_data_local_var = event_notify_data_create(
		gpsi &&
...
event_notify_data_t *event_notify_data_copy(event_notify_data_t *dst,
	...
	item = event_notify_data_convertToJSON(src);
	if(!item) {
		printf("event_notify_data_convertToJSON() failed");
		return NULL;
	}
	...
	event_notify_data_free(dst);
	dst = event_notify_data_parseFromJSON(item);
	cJSON_Delete(item);
