...
char *event_notify_data_type_ToString(
	...
	const char *event_notify_data_typeArray[] =
	{ "NULL", "UE_AVAILABLE", "PERIODIC", "ENTERING_INTO_AREA",
	  ...
	  "UE_MOBILITY_FOR_DEFERRED_LOCATION", "5GC_MT_LR",
	  "DIRECT_REPORT_EVENT", "CUMULATIVE_EVENT_REPORT" };
	size_t sizeofArray = sizeof(event_notify_data_typeArray) /
	                     sizeof(event_notify_data_typeArray[0]);
	if(event_notify_data_type < sizeofArray) {
		return (char *) event_notify_data_typeArray[
...
event_notify_data_type_e event_notify_data_type_FromString(
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(event_notify_data_type,
		          event_notify_data_typeArray[stringToReturn]) == 0) {
