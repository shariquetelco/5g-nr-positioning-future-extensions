...
char *supported_gad_shapes_ToString(supported_gad_shapes_e supported_gad_shapes)
{
	const char *supported_gad_shapesArray[] =
	{ "NULL", "POINT", "POINT_UNCERTAINTY_CIRCLE",
	  ...
	  "LOCAL_3D_POINT_UNCERTAINTY_ELLIPSOID", "DISTANCE_DIRECTION",
	  "RELATIVE_2D_LOCATION_UNCERTAINTY_ELLIPSE",
	  "RELATIVE_3D_LOCATION_UNCERTAINTY_ELLIPSOID" };
	size_t sizeofArray = sizeof(supported_gad_shapesArray) /
	                     sizeof(supported_gad_shapesArray[0]);
...
supported_gad_shapes_e supported_gad_shapes_FromString(
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(supported_gad_shapes,
		          supported_gad_shapesArray[stringToReturn]) == 0) {
