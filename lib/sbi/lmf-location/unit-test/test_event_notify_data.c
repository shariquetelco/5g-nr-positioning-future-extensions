...
event_notify_data_t *instantiate_event_notify_data(int include_optional) {
	...
	if(include_optional) {
		event_notify_data = event_notify_data_create(
			lmf_location_event_notify_data__PERIODIC_EVENT,
			"a",
			"a",
			"0",
			"01",
			...
			list_createList(),
			"0",
			lmf_location_event_notify_data__TERMINATION_BY_UE,
			null,
			-32767,
			// false, not to have infinite recursion
			instantiate_minor_location_qo_s(0),
			"a",
			lmf_location_event_notify_data__INDOOR,
			// false, not to have infinite recursion
			instantiate_high_accuracy_gnss_metrics(0),
			lmf_location_event_notify_data__LOS,
			56,
			"0",
			// false, not to have infinite recursion
