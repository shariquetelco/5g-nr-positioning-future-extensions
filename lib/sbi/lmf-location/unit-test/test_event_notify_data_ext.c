...
event_notify_data_ext_t *instantiate_event_notify_data_ext(int include_optional)
{
	...
	if(include_optional) {
		event_notify_data_ext = event_notify_data_ext_create(
			lmf_location_event_notify_data_ext__PERIODIC_EVENT,
			"a",
			"a",
			"0",
			"01",
			...
			list_createList(),
			"0",
			lmf_location_event_notify_data_ext__TERMINATION_BY_UE,
			null,
			-32767,
			// false, not to have infinite recursion
			instantiate_minor_location_qo_s(0),
			"a",
			lmf_location_event_notify_data_ext__INDOOR,
			// false, not to have infinite recursion
			instantiate_high_accuracy_gnss_metrics(0),
			lmf_location_event_notify_data_ext__LOS,
			56,
			"0",
			// false, not to have infinite recursion
