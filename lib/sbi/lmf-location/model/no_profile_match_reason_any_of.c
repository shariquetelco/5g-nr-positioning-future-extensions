...
char *no_profile_match_reason_any_of_ToString(
	...
	{ "NULL", "REQUESTER_PLMN_NOT_ALLOWED", "TARGET_NF_SUSPENDED",
	  "TARGET_NF_UNDISCOVERABLE", "QUERY_PARAMS_COMBINATION_NO_MATCH",
	  "TARGET_NF_TYPE_NOT_SUPPORTED", "UNSPECIFIED" };
	size_t sizeofArray = sizeof(no_profile_match_reason_any_ofArray) /
	                     sizeof(no_profile_match_reason_any_ofArray[0]);
	if(no_profile_match_reason_any_of < sizeofArray) {
		return (char *) no_profile_match_reason_any_ofArray[
...
no_profile_match_reason_any_of_e no_profile_match_reason_any_of_FromString(
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(no_profile_match_reason_any_of,
		          no_profile_match_reason_any_ofArray[stringToReturn])
		   == 0) {
