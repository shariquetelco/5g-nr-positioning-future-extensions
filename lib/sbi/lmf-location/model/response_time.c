...
char *response_time_ToString(response_time_e response_time) {
	const char *response_timeArray[] =
	{ "NULL", "LOW_DELAY", "DELAY_TOLERANT", "NO_DELAY" };
	size_t sizeofArray = sizeof(response_timeArray) /
	                     sizeof(response_timeArray[0]);
...
response_time_e response_time_FromString(char *response_time) {
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(response_time,
		          response_timeArray[stringToReturn]) == 0) {
