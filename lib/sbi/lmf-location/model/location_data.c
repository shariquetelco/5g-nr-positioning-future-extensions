...
location_data_t *location_data_create(geographic_area_t *location_estimate,
	...
	location_data_local_var->is_barometric_pressure =
...
void location_data_free(location_data_t *location_data) {
	...
	if(location_data->velocity_estimate) {
		velocity_estimate_free(location_data->velocity_estimate);
	...
	if(location_data->ecgi) {
		ecgi_free(location_data->ecgi);
		location_data->ecgi = NULL;
	}
	if(location_data->ncgi) {
		ncgi_free(location_data->ncgi);
	...
	if(location_data->ranging_sl_capability) {
		free(location_data->ranging_sl_capability);
		location_data->ranging_sl_capability = NULL;
	}
	free(location_data);
}
...
cJSON *location_data_convertToJSON(location_data_t *location_data) {
	...
	if(location_data->velocity_estimate) {
		cJSON *velocity_estimate_local_JSON =
			velocity_estimate_convertToJSON(
				location_data->velocity_estimate);
		if(velocity_estimate_local_JSON == NULL) {
			printf(
				"location_data_convertToJSON() failed [velocity_estimate]");
			goto end;
		}
	...
	if(location_data->ecgi) {
		cJSON *ecgi_local_JSON =
			ecgi_convertToJSON(location_data->ecgi);
		if(ecgi_local_JSON == NULL) {
			printf("location_data_convertToJSON() failed [ecgi]");
			goto end;
		}
	...
	if(location_data->ncgi) {
		cJSON *ncgi_local_JSON =
			ncgi_convertToJSON(location_data->ncgi);
		if(ncgi_local_JSON == NULL) {
			printf("location_data_convertToJSON() failed [ncgi]");
			goto end;
		}
	...
	if(location_data->is_barometric_pressure) {
		if(cJSON_AddNumberToObject(item, "barometricPressure",
		                           location_data->barometric_pressure)
		   == NULL)
		{
			printf(
				"location_data_convertToJSON() failed [barometric_pressure]");
			goto end;
		}
	}
	...
	if(location_data->ue_positioning_cap) {
		if(cJSON_AddStringToObject(item, "uePositioningCap",
		                           location_data->ue_positioning_cap) ==
		   NULL)
		{
			printf(
				"location_data_convertToJSON() failed [ue_positioning_cap]");
			goto end;
		}
	}
	...
	if(location_data->ue_area_ind) {
		cJSON *ue_area_ind_local_JSON =
			ue_area_indication_convertToJSON(
				location_data->ue_area_ind);
		if(ue_area_ind_local_JSON == NULL) {
			printf(
				"location_data_convertToJSON() failed [ue_area_ind]");
			goto end;
		}
	...
	if(location_data->supported_features) {
		if(cJSON_AddStringToObject(item, "supportedFeatures",
		                           location_data->supported_features) ==
		   NULL)
		{
			printf(
				"location_data_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(location_data->ranging_sl_capability) {
		if(cJSON_AddStringToObject(item, "rangingSlCapability",
		                           location_data->ranging_sl_capability)
		   == NULL)
		{
			printf(
				"location_data_convertToJSON() failed [ranging_sl_capability]");
			goto end;
		}
	}
...
location_data_t *location_data_parseFromJSON(cJSON *location_dataJSON) {
	...
	if(!location_estimate) {
		printf(
			"location_data_parseFromJSON() failed [location_estimate]");
		goto end;
	}
	...
	velocity_estimate = cJSON_GetObjectItemCaseSensitive(location_dataJSON,
	                                                     "velocityEstimate");
	...
	barometric_pressure = cJSON_GetObjectItemCaseSensitive(
		location_dataJSON, "barometricPressure");
	if(barometric_pressure) {
		if(!cJSON_IsNumber(barometric_pressure)) {
			printf(
				"location_data_parseFromJSON() failed [barometric_pressure]");
			goto end;
		}
	}
	...
	ue_positioning_cap = cJSON_GetObjectItemCaseSensitive(location_dataJSON,
	                                                      "uePositioningCap");
	if(ue_positioning_cap) {
		...
		{
			printf(
				"location_data_parseFromJSON() failed [ue_positioning_cap]");
			goto end;
		}
	}

	ue_area_ind = cJSON_GetObjectItemCaseSensitive(location_dataJSON,
	                                               "ueAreaInd");
	if(ue_area_ind) {
	...
	supported_features = cJSON_GetObjectItemCaseSensitive(location_dataJSON,
	                                                      "supportedFeatures");
	if(supported_features) {
		...
		{
			printf(
				"location_data_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}
