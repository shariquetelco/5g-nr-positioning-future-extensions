...
cJSON *event_notify_data_convertToJSON(event_notify_data_t *event_notify_data) {
	...
	if(event_notify_data->reported_event_type == reported_event_type_NULL) {
		printf(
			"event_notify_data_convertToJSON() failed [reported_event_type]");
		return NULL;
	}
	...
	if(event_notify_data->hgmlc_call_back_uri) {
		...
		{
			printf(
				"event_notify_data_convertToJSON() failed [hgmlc_call_back_uri]");
			goto end;
		}
	}
	...
	if(event_notify_data->lir_reference) {
		...
		{
			printf(
				"event_notify_data_convertToJSON() failed [lir_reference]");
			goto end;
		}
	}
	...
	if(event_notify_data->serving_lm_fidentification) {
		...
		{
			printf(
				"event_notify_data_convertToJSON() failed [serving_lm_fidentification]");
			goto end;
		}
	}
	...
	if(event_notify_data->supported_features) {
		...
		{
			printf(
				"event_notify_data_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
...
event_notify_data_t *event_notify_data_parseFromJSON(
	...
	reported_event_type = cJSON_GetObjectItemCaseSensitive(
		event_notify_dataJSON, "reportedEventType");
	if(!reported_event_type) {
		printf(
			"event_notify_data_parseFromJSON() failed [reported_event_type]");
		goto end;
	}
	...
	hgmlc_call_back_uri = cJSON_GetObjectItemCaseSensitive(
		event_notify_dataJSON, "hgmlcCallBackURI");
	if(hgmlc_call_back_uri) {
		if(!cJSON_IsString(hgmlc_call_back_uri) &&
		   !cJSON_IsNull(hgmlc_call_back_uri))
		{
			printf(
				"event_notify_data_parseFromJSON() failed [hgmlc_call_back_uri]");
			goto end;
		}
	}
	...
	if(lir_reference) {
		...
		{
			printf(
				"event_notify_data_parseFromJSON() failed [lir_reference]");
			goto end;
		}
	}
	...
	supported_features = cJSON_GetObjectItemCaseSensitive(
		event_notify_dataJSON, "supportedFeatures");
	if(supported_features) {
		if(!cJSON_IsString(supported_features) &&
		   !cJSON_IsNull(supported_features))
		{
			printf(
				"event_notify_data_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}
