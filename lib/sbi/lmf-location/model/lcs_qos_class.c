...
char *lcs_qos_class_ToString(lcs_qos_class_e lcs_qos_class) {
	const char *lcs_qos_classArray[] =
	{ "NULL", "BEST_EFFORT", "ASSURED", "MULTIPLE_QOS" };
	size_t sizeofArray = sizeof(lcs_qos_classArray) /
	                     sizeof(lcs_qos_classArray[0]);
...
lcs_qos_class_e lcs_qos_class_FromString(char *lcs_qos_class) {
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(lcs_qos_class,
		          lcs_qos_classArray[stringToReturn]) == 0) {
