...
{
	const char *dispersion_ordering_criterion_any_ofArray[] =
	{ "NULL", "TIME_SLOT_START", "DISPERSION", "CLASSIFICATION", "RANKING",
	  "PERCENTILE_RANKING" };
	size_t sizeofArray = sizeof(dispersion_ordering_criterion_any_ofArray) /
	                     sizeof(dispersion_ordering_criterion_any_ofArray[0]);
	if(dispersion_ordering_criterion_any_of < sizeofArray) {
		return (char *) dispersion_ordering_criterion_any_ofArray[
...
dispersion_ordering_criterion_any_of_FromString(
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(dispersion_ordering_criterion_any_of,
		          dispersion_ordering_criterion_any_ofArray[
				  stringToReturn]) ==
		   0) {
