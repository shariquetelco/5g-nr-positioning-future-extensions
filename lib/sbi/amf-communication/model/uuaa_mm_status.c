...
char *uuaa_mm_status_ToString(uuaa_mm_status_e uuaa_mm_status) {
	const char *uuaa_mm_statusArray[] =
	{ "NULL", "SUCCESS", "PENDING", "FAILED" };
	size_t sizeofArray = sizeof(uuaa_mm_statusArray) /
	                     sizeof(uuaa_mm_statusArray[0]);
...
uuaa_mm_status_e uuaa_mm_status_FromString(char *uuaa_mm_status) {
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(uuaa_mm_status,
		          uuaa_mm_statusArray[stringToReturn]) == 0) {
