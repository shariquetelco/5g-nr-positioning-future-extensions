...
scheduled_communication_time_t *scheduled_communication_time_create(
	...
	scheduled_communication_time_t *scheduled_communication_time_local_var =
		malloc(sizeof(scheduled_communication_time_t));

	scheduled_communication_time_local_var->days_of_week = days_of_week;
	scheduled_communication_time_local_var->time_of_day_start =
		time_of_day_start;
	scheduled_communication_time_local_var->time_of_day_end =
		time_of_day_end;
...
cJSON *scheduled_communication_time_convertToJSON(
	...
	if(scheduled_communication_time == NULL) {
		printf(
			"scheduled_communication_time_convertToJSON() failed [ScheduledCommunicationTime]");
		return NULL;
	}
	...
	if(scheduled_communication_time->days_of_week) {
		...
		if(days_of_weekList == NULL) {
			printf(
				"scheduled_communication_time_convertToJSON() failed [days_of_week]");
			goto end;
		}
		list_for_each(scheduled_communication_time->days_of_week,
		              node) {
	...
	if(scheduled_communication_time->time_of_day_start) {
		...
		{
			printf(
				"scheduled_communication_time_convertToJSON() failed [time_of_day_start]");
			goto end;
		}
	}
	...
	if(scheduled_communication_time->time_of_day_end) {
		...
		{
			printf(
				"scheduled_communication_time_convertToJSON() failed [time_of_day_end]");
			goto end;
		}
	}
...
scheduled_communication_time_t *scheduled_communication_time_parseFromJSON(
	...
	days_of_week = cJSON_GetObjectItemCaseSensitive(
		scheduled_communication_timeJSON, "daysOfWeek");
	if(days_of_week) {
		cJSON *days_of_week_local = NULL;
		if(!cJSON_IsArray(days_of_week)) {
			printf(
				"scheduled_communication_time_parseFromJSON() failed [days_of_week]");
			goto end;
		}
	...
	time_of_day_start = cJSON_GetObjectItemCaseSensitive(
		scheduled_communication_timeJSON, "timeOfDayStart");
	if(time_of_day_start) {
		if(!cJSON_IsString(time_of_day_start) &&
		   !cJSON_IsNull(time_of_day_start))
		{
			printf(
				"scheduled_communication_time_parseFromJSON() failed [time_of_day_start]");
			goto end;
		}
	}

	time_of_day_end = cJSON_GetObjectItemCaseSensitive(
		scheduled_communication_timeJSON, "timeOfDayEnd");
	if(time_of_day_end) {
		if(!cJSON_IsString(time_of_day_end) &&
		   !cJSON_IsNull(time_of_day_end))
		{
			printf(
				"scheduled_communication_time_parseFromJSON() failed [time_of_day_end]");
			goto end;
		}
	}
...
scheduled_communication_time_t *scheduled_communication_time_copy(
	...
	item = scheduled_communication_time_convertToJSON(src);
	if(!item) {
		printf("scheduled_communication_time_convertToJSON() failed");
		return NULL;
	}
	...
	scheduled_communication_time_free(dst);
	dst = scheduled_communication_time_parseFromJSON(item);
	cJSON_Delete(item);
