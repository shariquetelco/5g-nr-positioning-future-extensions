...
char *wlan_ordering_criterion_any_of_ToString(
	...
	const char *wlan_ordering_criterion_any_ofArray[] =
	{ "NULL", "TIME_SLOT_START", "NUMBER_OF_UES", "RSSI", "RTT",
	  "TRAFFIC_INFO" };
	size_t sizeofArray = sizeof(wlan_ordering_criterion_any_ofArray) /
	                     sizeof(wlan_ordering_criterion_any_ofArray[0]);
	if(wlan_ordering_criterion_any_of < sizeofArray) {
		return (char *) wlan_ordering_criterion_any_ofArray[
...
wlan_ordering_criterion_any_of_e wlan_ordering_criterion_any_of_FromString(
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(wlan_ordering_criterion_any_of,
		          wlan_ordering_criterion_any_ofArray[stringToReturn])
		   == 0) {
