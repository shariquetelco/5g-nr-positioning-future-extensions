...
char *expected_analytics_type_any_of_ToString(
	...
	{ "NULL", "MOBILITY", "COMMUN", "MOBILITY_AND_COMMUN" };
	size_t sizeofArray = sizeof(expected_analytics_type_any_ofArray) /
	                     sizeof(expected_analytics_type_any_ofArray[0]);
	if(expected_analytics_type_any_of < sizeofArray) {
		return (char *) expected_analytics_type_any_ofArray[
...
expected_analytics_type_any_of_e expected_analytics_type_any_of_FromString(
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(expected_analytics_type_any_of,
		          expected_analytics_type_any_ofArray[stringToReturn])
		   == 0) {
