...
char *time_unit_any_of_ToString(time_unit_any_of_e time_unit_any_of) {
	const char *time_unit_any_ofArray[] =
	{ "NULL", "MINUTE", "HOUR", "DAY" };
	size_t sizeofArray = sizeof(time_unit_any_ofArray) /
	                     sizeof(time_unit_any_ofArray[0]);
...
time_unit_any_of_e time_unit_any_of_FromString(char *time_unit_any_of) {
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(time_unit_any_of,
		          time_unit_any_ofArray[stringToReturn]) == 0) {
