...
char *accuracy_any_of_ToString(accuracy_any_of_e accuracy_any_of) {
	const char *accuracy_any_ofArray[] =
	{ "NULL", "LOW", "MEDIUM", "HIGH", "HIGHEST" };
	size_t sizeofArray = sizeof(accuracy_any_ofArray) /
	                     sizeof(accuracy_any_ofArray[0]);
...
accuracy_any_of_e accuracy_any_of_FromString(char *accuracy_any_of) {
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(accuracy_any_of,
		          accuracy_any_ofArray[stringToReturn]) == 0) {
