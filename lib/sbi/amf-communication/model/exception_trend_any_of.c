...
char *exception_trend_any_of_ToString(
	...
	const char *exception_trend_any_ofArray[] =
	{ "NULL", "UP", "DOWN", "UNKNOW", "STABLE" };
	size_t sizeofArray = sizeof(exception_trend_any_ofArray) /
	                     sizeof(exception_trend_any_ofArray[0]);
	if(exception_trend_any_of < sizeofArray) {
		return (char *) exception_trend_any_ofArray[
...
exception_trend_any_of_e exception_trend_any_of_FromString(
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(exception_trend_any_of,
		          exception_trend_any_ofArray[stringToReturn]) == 0) {
