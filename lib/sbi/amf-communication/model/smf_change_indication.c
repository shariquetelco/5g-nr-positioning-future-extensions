...
char *smf_change_indication_ToString(
	...
	const char *smf_change_indicationArray[] =
	{ "NULL", "CHANGED", "REMOVED" };
	size_t sizeofArray = sizeof(smf_change_indicationArray) /
	                     sizeof(smf_change_indicationArray[0]);
...
smf_change_indication_e smf_change_indication_FromString(
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(smf_change_indication,
		          smf_change_indicationArray[stringToReturn]) == 0) {
