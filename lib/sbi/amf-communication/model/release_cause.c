...
char *release_cause_ToString(release_cause_e release_cause) {
	const char *release_causeArray[] =
	{ "NULL", "SNPN_SNPN_MOBILITY", "NO_HR_AGREEMENT", "UNSPECIFIED" };
	size_t sizeofArray = sizeof(release_causeArray) /
	                     sizeof(release_causeArray[0]);
...
release_cause_e release_cause_FromString(char *release_cause) {
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(release_cause,
		          release_causeArray[stringToReturn]) == 0) {
