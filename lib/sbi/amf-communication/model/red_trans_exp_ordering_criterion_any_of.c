...
{
	const char *red_trans_exp_ordering_criterion_any_ofArray[] =
	{ "NULL", "TIME_SLOT_START", "RED_TRANS_EXP" };
	size_t sizeofArray =
		sizeof(red_trans_exp_ordering_criterion_any_ofArray) /
		sizeof(red_trans_exp_ordering_criterion_any_ofArray[0]);
	if(red_trans_exp_ordering_criterion_any_of < sizeofArray) {
		return (char *) red_trans_exp_ordering_criterion_any_ofArray[
...
red_trans_exp_ordering_criterion_any_of_FromString(
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(red_trans_exp_ordering_criterion_any_of,
		          red_trans_exp_ordering_criterion_any_ofArray[
				  stringToReturn]) ==
		   0) {
