...
{
	const char *dataset_statistical_property_any_ofArray[] =
	{ "NULL", "UNIFORM_DIST_DATA", "NO_OUTLIERS" };
	size_t sizeofArray = sizeof(dataset_statistical_property_any_ofArray) /
	                     sizeof(dataset_statistical_property_any_ofArray[0]);
	if(dataset_statistical_property_any_of < sizeofArray) {
		return (char *) dataset_statistical_property_any_ofArray[
...
dataset_statistical_property_any_of_FromString(
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(dataset_statistical_property_any_of,
		          dataset_statistical_property_any_ofArray[
				  stringToReturn]) ==
		   0) {
