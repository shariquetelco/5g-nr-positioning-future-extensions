...
char *periodic_communication_indicator_ToString(
	...
	const char *periodic_communication_indicatorArray[] =
	{ "NULL", "PIORIODICALLY", "ON_DEMAND" };
	size_t sizeofArray = sizeof(periodic_communication_indicatorArray) /
	                     sizeof(periodic_communication_indicatorArray[0]);
	if(periodic_communication_indicator < sizeofArray) {
		return (char *) periodic_communication_indicatorArray[
...
periodic_communication_indicator_e periodic_communication_indicator_FromString(
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(periodic_communication_indicator,
		          periodic_communication_indicatorArray[stringToReturn])
		   == 0) {
